...
export class CategoryComponent implements OnInit {
  ...
  gaoDeMapStart(data): void {
    this.map = new AMap.Map('categoyMap', {
  ...
  selectCity(event: any): void {
    if (event.data) {
      ...
      this.currentCity = { 'name': event.name, link: '/content/lockTotal', 'areasId': event.data.data[0]['url'], 'rank': this.rank, 'size': this.size, 'levenl': 'city' };
        ...
        formatter: function (params) {
          ...
          if (params.data) {
            ...
            for (let item in params.data.data) {
              title += params.data.data[item]['level'] + "：" + params.data.data[item]['count'] + "个<br>";
            }
            return title
          }
        }
  ...
  addMarker(markers: any): void {
    ...
    markers.forEach((event) => {
      ...
      AMap.convertFrom(path, 'baidu', function (status, result) {
        ...
        for(let item in event.data){
          title += event.data[item]['level'] + '：' + event.data[item]['count'] + '，'
        }
        markerion.setTitle(title);
        that.marker.push(markerion);

        // 点击事件
        markerion.on('click', (item) => {
          console.log(event.data[0]['count']);
  ...
  cityId(event): void {
    ...
    this.district.search(event.areasId, (status, result) => {
      if (status === 'complete') {
        ...
        this.currentCity = { 'name': event.name, link: '/content/lockTotal', 'areasId': event.areasId, 'rank': this.rank, 'size': this.size, 'levenl': event.levenl };
        this.searchCity(this.currentCity.areasId);
        this.checkedBack.emit(this.currentCity); // 调用父级方法
      }
    });
